...
A set of research guidelines in your instruction file that Jesse follows whenever research is requested. Every research task produces a scoped, cited report with an actionability section tied to whatever triggered it.
...
Create `Projects/Research/` if it doesn't exist. Each research task produces one file here, named descriptively: `Apple-Silicon-Comparison.md`, `Home-Insurance-Options.md`, etc.
...
When research is requested (from Inbox/, conversation, or any other trigger), follow these guidelines for all research output. Research output is for human consumption. It must be accurate, but also accessible -- written so the reader can come away understanding the topic, not just holding a pile of citations.
...
1. Drop a note in Inbox/ saying "research X" or ask Jesse directly.
2. Jesse scopes the research, gathers sources, and writes a structured report.

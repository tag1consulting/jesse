...
- **Brief subagents with the guidelines they need.** When delegating a task that produces user-facing output -- research reports, drafts, weekly summaries, archive processing, meeting agendas -- the subagent prompt must explicitly tell it to read JESSE.md (or CLAUDE.md) plus the specific `Knowledge/Jesse-Guidelines/` files relevant to the task. The parent picks the list; do not make the subagent guess what applies. Without this, subagents drift from voice, formatting, and footer conventions. Carve-out: narrow lookups (a single grep, a glob search, "find me X in the vault") skip this -- reading the instruction file for one search is pure overhead. Rule of thumb: if the subagent is producing something the user will read, it loads the guidelines; if it is producing a path or a count, it does not.
- **Compaction restores guidelines.** JESSE.md (or CLAUDE.md) survives compaction because it is in the system prompt. Guideline files loaded via Read during the session do not -- and after compaction the agent has no other way to know what is gone. Every compaction summary must therefore include a `**Guidelines in active use:**` section listing every `Knowledge/Jesse-Guidelines/` file that was loaded during the pre-compaction session. The first post-compaction action is to re-read every file in that list before doing anything else. Reference files (Projects/, People/, etc.) do not need to be re-loaded eagerly -- re-fetch them only if the task at hand calls for them.

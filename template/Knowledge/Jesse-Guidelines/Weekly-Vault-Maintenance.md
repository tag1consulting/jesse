...
- **Archive date prefixes** -- Check `Projects/drafts/archive/` and `Inbox/archive/` for files missing `YYYY-MM-DD-` prefixes. Add them. **Idempotency rule:** if a filename already starts with a `YYYY-MM-DD-` prefix (regex `^\d{4}-\d{2}-\d{2}-`), it counts as already-prefixed — do NOT add another date. Ignore `.gitkeep` files.
